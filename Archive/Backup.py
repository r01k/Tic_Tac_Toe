...
class Game:
    ...
    def right_click(self, event):
        ...
        # If no line-completing squares are found for the machine:
        if self.machine_click("machine") == "no candidates approved":
            ...
            if self.machine_click("user") == "no candidates approved":
                # Detects a match when 2 or fewer squares are empty after
                # the machine marks its square.
                if 9 - (len(user_marks) + len(machine_marks) + 1) <= 2:
                    self.display_result("none")
                    if machine_starts:
                        self.machine_click(choose_random=True)
                        return
                # Machine chooses a random square.
                self.machine_click(choose_random=True)
                # Checks whether the machine won.
                ...
                if 9 - (len(user_marks) + len(machine_marks) + 1) <= 2:
                    # If no line-completing squares are found for the machine:
                    if self.machine_click(candidates_for="machine", skip_marking=True) \
                            == "no candidates approved":
                        # If no line-completing square are found for the user:
                        if self.machine_click(candidates_for="user", skip_marking=True) \
                                == "no candidates approved":
                            # Detects a match when 2 or fewer squares are empty after
                            # the machine marks its square.
                            self.display_result("none")
                            if machine_starts:
                                self.machine_click(choose_random=True)
                                return
            # If a line-completing square was found for the user
            # checks whether the user won.
            if self.check_win("user"):
                self.display_result("user")
                # If it's the machine's turn to start:
                if machine_starts:
                    self.machine_click(choose_random=True)
                return
        # If a line-completing square was found for the machine:
        if self.check_win("machine"):
            self.display_result("machine")
            # If it's the machine's turn to start:
            if machine_starts:
    ...
    def machine_click(self, candidates_for="machine", choose_random=False, skip_marking=False):
        ...
        if choose_random:
            while 1:
                ...
                if str(chosen_square) not in str(machine_marks) and \
                    ...
                    print("Will choose a random square")
                    break
        else:
            exit_loop = False
        ...
        if not skip_marking:
            ...
            print(machine_marks, user_marks)###############
            print(9 - (len(user_marks) + len(machine_marks)))  ##############
